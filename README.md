...
| `AUTH_SECRET` | Секрет подписи сессий админки — **обязателен в проде** |
...
Панель — по адресу **`/admin`**, вход по email + паролю (сессия в HttpOnly-cookie,
подпись HMAC через `AUTH_SECRET`).

- Дефолтный админ из сида: `admin@trudkrutshop.ru` / `admin12345` — **сменить сразу**
  (в проде через кнопку «Пароль» в шапке админки).
- Роли (`AdminRole`): **ADMIN** — всё; **MANAGER** — заказы, склад, промокоды,
  сертификаты; **CONTENT** — товары и разделы. Недоступные разделы скрыты, а каждое
  действие проверяется на сервере.

## ☁️ Деплой на Vercel

Проект собирается на Vercel как обычное Next-приложение (standalone включается
только вне Vercel). Нужно:

1. Импортировать репозиторий (Add New → Project).
2. Задать переменные: `DATABASE_URL` (Supabase **transaction-pooler**, порт 6543,
   `?pgbouncer=true`) и `AUTH_SECRET`.
3. Deploy — далее автодеплой на каждый push в `main`.
